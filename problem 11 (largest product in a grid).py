grid=[[1]*20 for k in range(20)] #initialize the grid of numbers
...
lst=[] #the list of all possible products

for i in range(20):
	for j in range(20): #we loop through all the entries of the grid
		
		prod=[1]*8 #variable to compute all the products
		
		for k in range(4):  #for each entry of the grid we compute all 8 possible products
			if i>=3 and j<=3:
				prod[0]*=grid[i-k][j-k]
			if i>=3:
				prod[1]*=grid[i-k][j]
